...
**已跑出的结果（黄金集 v1 · 64 条 · 1121 篇语料）**

消融 #1 纯 Dense vs Hybrid（Dense+BM25+RRF）：

| 指标 | 纯 Dense | Hybrid | Δ |
|------|---------|--------|---|
| Recall@5 | 0.667 | **0.719** | +5.2pt |
| Recall@8 | 0.754 | **0.807** | +5.3pt |
| MRR | 0.597 | **0.612** | +1.5pt |

分题型（Recall@8）：fact 1.00→1.00 · open_discussion 1.00→1.00 · decision 0.75→0.88 ·
term 0.75→0.83 · time_filter 0.43→**0.57** · **cross_doc 0.38→0.38（未改善）**

→ 结论：Hybrid 增益集中在精确匹配类（decision / term / time_filter）；
**cross_doc 是检索范式问题**（需按 doc_id 聚合或元数据过滤），不是融合问题——下一阶段主攻方向。
