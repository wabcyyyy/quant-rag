...
**消融 #2 固定 512 切 vs 结构感知分块（结论：结构感知的优势未被证实）**

| 指标 | 固定 512 切 | 结构感知分块 | 判读 |
|------|------------|-------------|------|
| 块数 | 5541 | 3856 | — |
| Recall@5 / Recall@8 | **0.982 / 0.982** | 0.945 / 0.945 | 固定切分更优（块更细） |
| MRR | **0.804** | 0.785 | 固定切分略优 |
| 表格完整率 | 52.5%（85/179 被切断） | **100%** | 机制成立 |
| 包含匹配（63 条） | 0.800 | **0.818** | 差异在噪声内 |
| **RAGAS Faithfulness** | **0.821** | 0.641 | **与预期相反** |
| 表格相关题（11 条）包含匹配 | **0.909** | 0.818 | 机制未转化为收益 |
| 其余题（44 条）包含匹配 | 0.795 | **0.818** | 同上 |
