...
gold 文档数 > 检索预算，或答案需要两篇各记载一半。**原计划的三个新题型已经降到两类**
（2026-09-20 普查否证了「跨篇表格数值」，见 §1 第 3 条）：**时间线推翻** 与 **窗口依赖**。
时间线类的 gold 只取**后一次**决议、`must_contain` 落在后来那篇的关键句 —— 规则 3 不放宽时
「A 推翻了 B」这种合并陈述本来就答不出来，把题做成「要检索到对的哪一版」才是合法的读法。
零 LLM 构造优先（`gen-gold --programmatic-only` 那套）。
