...
| 3 | ~~表格数值题在黄金集里为 0 条，而「表格完整率 100% vs 52.5%」是头条~~ → **2026-09-20 普查否证**：106/1127 篇含 179 个表格，但含数值行的只有 29 个；「同一行标签跨 ≥2 篇」raw 13 个里 8 个是日期行、4 个是序号列，**只剩 1 个**（入职日期，2 篇）→ 凑不出 8 条跨篇数值对比题，这条动机不成立 | `doc-rag census-corpus`（¥0，只读 parsed）；表格被切断那半仍成立（PLAN §5.3：85/179），但它不是 agent 的理由 |
...
2. ✅ agent mode 的追加断言已落（2026-09-20）：**trace 步数、每步 action 序列、停机原因、判定
   调用次数、token 合计、并集文档数**五条入口逐项相等；采集点在 `run_agent` 那一层，
   所以「某条入口悄悄绕过 policy 层」表现为 traces 少一份。前提见本节第 6 条（确定性假 judge）。
...
新黄金集 **v3 multi-hop**（独立文件，不与现有 72 条混），**按构造保证单条清单必败**：
gold 文档数 > 检索预算，或答案需要两块各记载一半。
✅ **2026-09-20 实测下来只有一类成立，而且只有 3 条原料**：三个题型里，跨篇表格数值被
`doc-rag census-corpus` 否证（可对齐的跨篇标签只剩 1 个）；时间线推翻不满足入场券
（gold 只有 1 篇文档，考的是 recency/排序 —— 「gold 只取后一次决议」这条口径继续有效，
但它属于 §1 第 4 条那条**不需要 agent** 的线，不在 v3 里）；**只有窗口依赖成立**
（`gen-gold --v3`：值只在 B 块、主语只在 A 块，5 个候选里带决议线索词且主语不是模板词的 3 个，
性质复检 `v3_property_violations` 随文件落盘）。
→ v3 要成规模只能**造料**（扩 `scripts/make_sample_corpus.py` 生成这个形状，或 LLM 出题 +
逐字 grounding 校验）。零 LLM 构造优先（`gen-gold --programmatic-only` 那套）。
...
| **P0.5** | ① E2 上下文预算消融（go/no-go，见 §8）· ② ✅ **已跑完（¥0，`census-corpus`）**：179 个表格分布在 106 篇，可对齐的跨篇数值标签只剩 1 个 → 第 3 条动机不成立，v3 题型降到两类 | ①Qdrant+key ②无 | ①≈¥2~4 ②0 |
...
6. §4 的 parity 扩展在 mock 下不可实现，除非注入**确定性假 judge**（脚本化判决序列），否则五入口的
   ...
   ✅ 已照此落地（2026-09-20）：`judge` fixture 脚本化判决 + 每条入口重置游标。
