...
### 7.1 违规实例（每条规则都对应一次真实踩坑——这是本协议的可迁移部分）

| 规则 | 违反会怎样（真实案例） |
|---|---|
| 1 同题配对 | 拿 **v1（63 条）与 v2（72 条）**两个黄金集的 `contains_acc` 并排读，差值里混着题集差——v2 的 time_filter 上限从 15 提到 40，「同一条口径」的说法当场失效（PLAN §5.3 记了这次教训）。 |
| 2 95%CI | 只报均值差不报 CI：`无重排 vs 有重排` 的答案侧差 0.5pt 看像「重排有害」，CI 一算跨 0，实为无差异。 |
| 3 符号检验 | 只看 CI 不看赢/输方向：某消融 CI 略偏正但赢/输 = 4/6（方向相反）→ 那不是「小幅改进」，是抖动。 |
| 4 Holm 家族 | **分两次跑各挑一个显著**：本仓库实测过 `p<0.05` 在 20 个比较里迟早出现（PLAN §5.3 的四臂×六指标 = 20 检验）。**2026-09-26 C5 是活例**：nocite 臂原始 p=0.0386、进 108 比较家族后 p_holm=1.0——「显著」是家族的函数，不是单条比较的属性。 |
| 5 `unscored` | 把「某臂没有该判据」当 0 计入家族：旧结果文件没有 `keypoint_hit`，算进去就会稀释别的指标的 α。 |
| 6 等长护栏 | **重排第一版**的 `nDCG@8 −3.4pt / 覆盖率 −1.6pt`——全是「6 格 vs 8 格清单」造出来的伪影，不是排序质量差；同一条病在答案侧是 N3 的「8 块 vs 6 块」（README:279 已加条件）。 |
| 7 显著 ≠ 值得 | `--repeat` 之外的**抽样偏置**：早期消融用 `rows[:15]` 抽样，恰好整段漏掉排在表尾的 cross_doc/time_filter，18pt 的「差异」是抽样造出来的；**2026-09-26 又一例**：keypoint 地板 7.1pt 之下，C5 九个臂里七个的差都「存在」但不值得读。 |

> **可迁移的三句话**：① 任何差值先问「同题吗、等长吗、进同一家族了吗」；
> ② 过了统计这关再问「过地板吗」；③ 过了地板再问「这是判据可被绕开还是质量真变好」
> （C5 的 nocite +22.6pt 就是第③问的反例：去掉引用约束后答案变长更容易碰巧命中要点）。

---

## 8. 三条噪声地板

**现行值（2026-09-26 C 电池在公开底座 320 篇 / 冻结 `1d53ea329bf4` 上重测）**：

| 指标 | 地板（新底座） | 怎么量出来的 | 实测日期 |
|---|---|---|---|
| doc 级检索指标 | **0~2 条（Hit@5 ≤1.1pt）** | `--retrieval-only --repeat 2`：gold_core(74) 逐条 0 翻转、Hit@5 极差 0；gold_full(198) 翻转 2 条、Hit@5 极差 1.05pt、MRR 0.17pt | 2026-09-26 |
| `faithfulness` | **0.4pt** | 同一批答案（n=66）、同一 judge、temperature=0、关缓存重判一次：0.9813 → 0.9853 | 2026-09-26 |
| `keypoint_recall`（聚合子集） | **7.1pt** | `--repeat 2 --fresh-answers`（gold_core 74，5/31 条翻转）：0.5984 → 0.6694；同运行 `strict_keyword_accuracy` 极差 1.5pt（1 条） | 2026-09-26 |

**⚠ keypoint 地板从 3.10pt 涨到 7.1pt（2.3 倍）**——聚合题 keypoint 的 run-to-run
抖动在新底座上远大于旧公司语料。含义：C5 消融里除 nocite（+22.6pt）与 norerank
（+10.1pt）外的全部答案侧差值都落在这个地板内，**不得下结论**。

**旧值（公司语料 v1 口径，已作废，仅存档）**：

| 指标 | 旧地板 | 实测日期 |
|---|---|---|
| doc 级检索指标 | 0（库未重嵌入）/ 重嵌入后 ±2 条 = ±3.1pt | 2026-09-19（修订 09-23） |
| `faithfulness` | ≥1.9pt | 2026-09-19 |
| `keypoint_hit_ratio` | 3.10pt | 2026-09-21 |

三条**不可互换**：judge 地板不适用于答案轨，答案轨地板不适用于检索轨。
另注意「重跑地板 = 0」只说检索指标是确定性的，它同时意味着——**答案侧的 run-to-run 差异不会有任何检索指标先报警**；所以**任何「检索基线重发」必须带同状态重跑极差，单遍读数不再单独具有结论资格**。
...
- **§8 的三条噪声地板已于 2026-09-26 在公开底座重测替换**（旧值只作存档）：
  检索极差 0~2 条、faithfulness 0.4pt、**keypoint 7.1pt**（从 3.10pt 涨到 2.3 倍）。
  三条旧值测于公司语料（v1 口径），**不迁移**——本次重测证实了这一点。
- 当前的参照读数 = PLAN §5.7 的阶段读数表（S0~S3 + A3.4 三臂，全部检索侧零 LLM）
  与 C 电池读数表（`docs/ops/c-battery-20260926.md`，本地）。

**违反会怎样（实例）**：

| 违反 | 后果（真实案例） |
|---|---|
| 跨 `freeze_id` 并排报数 | 公司语料时代的答案基线（0.875 / 0.9264）与新公开底座的 0.9394 若并排，读者会读成「+6pt 改进」——实际是**换了语料、换了题集、换了索引**三件事同时发生。2026-09-26 README 收口就是按这条把两批数拆成了「头条数字（公开语料）」与「历史基线（不可复核）」。 |
| 忽略 `freeze_warnings` | `eval` 的 meta 对**只相关的那一个**指纹报警（检索看 `index_fp`、答案看 `synth_fp`）。一次只改 prompt 的运行如果带着 `index_fp` 报警一起忽略，会把「答案侧该重跑」误判成「检索侧作废」——范围判错，多花一次全量重嵌入的钱。 |
| 在 `index_fp` 相同的两个库之间比检索 | 同语料同分块的全量重嵌入**不改变** `index_fp`（7 项输入全是结构元数据，2026-09-26 审查 N5 已定级为待修），但实测能让 14/72 条检回集合不同、Hit@5 摆 ±2 条 → 这种「同指纹不同向量」的比较是**指纹机制当前看不见的**违规。 |
| 用未修完机制的底座叠新结论 | 公司语料索引丢 ≈43% 正文（README 已知不足第 1 条），而 `doc_profiler`/gold 自检当时都只看**解析文本**、看不见它在索引里缺——继续在它上面报「某机制 +Xpt」等于在坏底座上归因。 |
