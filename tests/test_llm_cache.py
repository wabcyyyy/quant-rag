...
def test_cache_key_differs_by_model_prompt_and_endpoint():
    ...
    k3 = llm._cache_key(
        _cfg(), [{"role": "user", "content": "另一个问题"}], temperature=0
    )
...
def test_cache_key_differs_by_endpoint_even_for_same_model():
    ...
    assert llm._cache_key(
        _cfg(base_url="https://api.a.com"), msgs, temperature=0
    ) != llm._cache_key(_cfg(base_url="https://api.b.com"), msgs, temperature=0)
...
def test_cache_roundtrip_and_stats(tmp_path, monkeypatch):
    ...
    key = llm._cache_key(
        _cfg(model="m"), [{"role": "user", "content": "q"}], temperature=0
    )
