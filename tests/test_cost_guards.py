...
def test_usage_accounted_and_reasoning_separated(chat_env):
    ...
    assert (st["prompt_tokens"], st["completion_tokens"], st["reasoning_tokens"]) == (
        10,
        5,
        3,
    )
...
def test_cache_write_error_counted(monkeypatch):
    _fresh_stats(monkeypatch)
    monkeypatch.setattr(
        llm_mod, "_conn", Mock(side_effect=RuntimeError("read-only fs"))
    )
...
def test_judge_cache_init_failure_blocks_uncached_full_run(monkeypatch):
    ...
    pytest.importorskip(
        "langchain_community.cache", reason="eval extra（langchain）未安装"
    )
    _fresh_stats(monkeypatch)
    cfg = {
        "llm": {"cache": True},
    ...
    monkeypatch.setattr(
        lc, "SQLiteCache", Mock(side_effect=RuntimeError("no .cache dir"))
    )
...
def test_counter_reads_raw_openai_reasoning_key():
    pytest.importorskip(
        "langchain_core.callbacks", reason="eval extra（langchain）未安装"
    )
...
def test_counter_accumulates_normalized_generations():
    ...
    d = c.as_dict()
    assert (d["prompt_tokens"], d["completion_tokens"], d["reasoning_tokens"]) == (
        7,
        9,
        8,
    )
...
def _write_results(tmp_path, n=20, contexts="legacy", recorded_rewrite=True):
    items = []
    for i in range(n):
        body = f"正文{i}"
        ...
        if recorded_rewrite:
            # meta.retrieval 带 +rewrite → 重放只能读条目记下的检索串
            item |= {
                "rewritten": f"问题{i}",
        ...
        json.dumps(
            {
                "meta": {
                    "collection": "c",
                    "retrieval": "dense+bm25+rrf[dense]+rewrite+rerank",
                    "top_n": 8,
                },
                "items": items,
            },
    ...
    class _Retriever:
        ...
        def retrieve(self, question, **kw):
            ...
            return RetrievalOutcome(
                chunks=[{"doc_id": "d", "title": "t", "page": 1, "text": text}]
            )

    monkeypatch.setattr(
        runner, "_build_retriever", lambda cfg, col: (_Retriever(cfg), None)
    )
...
def test_no_retriever_built_when_contexts_are_current(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        runner,
        "_build_retriever",
        Mock(side_effect=AssertionError("不应构建 retriever")),
    )
    judge = Mock(return_value={"faithfulness": 1.0})
    monkeypatch.setattr(runner, "_run_ragas", judge)
...
def test_context_mismatch_refuses_to_judge(tmp_path, monkeypatch):
    ...
    _, _modes = _patch_retrieval(
        monkeypatch, {"问题0": "别的正文", "问题1": "别的正文"}
    )
...
def test_replay_restores_saved_retrieval_mode(tmp_path, monkeypatch):
    ...
    path, _ = _write_results(tmp_path, n=2)
    seen, built_modes = _patch_retrieval(
        monkeypatch, {f"问题{i}": f"正文{i}" for i in range(2)}
    )
...
def _write_parsed(parsed_dir, n=5):
    for i in range(n):
        (parsed_dir / f"d{i}.json").write_text(
            json.dumps(
                {
...
def index_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        indexer.Embedder, "embed", lambda self, texts: [[0.0]] * len(texts)
    )
    monkeypatch.setattr(
        indexer,
        "chunk_by",
...
def test_index_honors_enabled_flag_and_explicit_optin(index_env, monkeypatch):
    ...
    extract.reset_mock()
    assert (
        indexer.index_parsed(index_env, cfg_on, use_llm_meta=False)["llm_meta"] is False
    )
    extract.assert_not_called()
...
def test_index_limit_bounds_indexing(index_env):
    ...
    assert sum(client.upserted) == 2  # 真的只 upsert 了 2 篇的块


def test_cache_has_an_inventory_and_a_clear_path(tmp_path, monkeypatch):
    """缓存必须可查、可作废。

    键里带完整 prompt（问题原文 + 检索到的正文），命中即返回：它让重复评估近乎
    零成本，同时也是语料内容在磁盘上的一份副本，并且会在 re-ingest 之后继续供
    旧答案。三件事都需要一个入口，而不是靠人回忆。
    """
    monkeypatch.setattr(llm_mod, "_CACHE_PATH", tmp_path / "cache.sqlite")
    llm_mod._cache_put("k1", "model-a", "resp1")
    llm_mod._cache_put("k2", "model-b", "resp2")

    inv = llm_mod.cache_inventory()
    assert inv["entries"] == 2
    assert inv["size_bytes"] > 0
    assert {r["model"] for r in inv["by_model"]} == {"model-a", "model-b"}

    assert llm_mod.cache_clear()["deleted"] == 2
    assert llm_mod.cache_inventory()["entries"] == 0
    assert llm_mod._cache_get("k1") is None  # 清完不能再命中
