...
def build_pdf(doc: dict, font: Path) -> bytes:
    ...
    leftover = page.insert_textbox(rect, text, fontname=fontname, fontfile=str(font),
                                   fontsize=11.5, lineheight=1.5, align=fitz.TEXT_ALIGN_LEFT)
    if leftover < 0:  # 单页装不下：续页（示例语料长度已控制，此分支为保险）
        ...
        page2.insert_textbox(fitz.Rect(56, 64, 595 - 56, 842 - 56), text[-int(leftover):],
                             fontname=fontname, fontfile=str(font), fontsize=11.5, lineheight=1.5)
    pdf.set_metadata({
    ...
    data = re.sub(rb"/ID\[<[0-9A-Fa-f]+><[0-9A-Fa-f]+>\]",
                  rb"/ID[<" + b"0" * 32 + rb"><" + b"0" * 32 + rb">]", data)
    return data
...
def main() -> None:
    ...
    for term in OFF_CORPUS_TERMS:
        assert term not in joined, f"域外词「{term}」出现在示例语料中，no_answer 题会失效"
    ...
    for item in gold["items"]:
        ...
        for kw in item["must_contain"]:
            if norm(kw) not in corpus:
                failures.append(f"{qid}({qtype}): must_contain「{kw}」未逐字命中来源文档")
    if failures:
